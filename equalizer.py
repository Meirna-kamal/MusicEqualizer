...
class MainApp(QMainWindow,MAIN_WINDOW):
    ...
    def spectroBefore(self,colorMap):
        fig = plt.figure()
        plt.subplot(111)
        self.powerSpectrum, self.freqenciesFound, self.time, self.imageAxis = plt.specgram(audioData, Fs=sampling_rate, cmap=colorMap)
    ...
    def spectroAfter(self):
        ...
        fig = plt.figure()
        plt.subplot(111)
        self.spectrogram = plt.specgram(adjusted_audio, Fs=sampling_rate, cmap= self.comap, scale='dB', vmin=self.min_value_slider.value(), vmax=self.max_value_slider.value())
        plt.xlabel('Time')
        plt.ylabel('Frequency')
        plt.colorbar()
